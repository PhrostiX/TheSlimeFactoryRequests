...
function statusBadge(status?: string) {
  ...
  if (low.includes("pending")) return { text: "Pending Review", style: styles.statusPending };
  if (low.includes("rated")) return { text: "★ Rated", style: styles.statusRated };
  if (low.includes("sending")) return { text: "Sending", style: styles.statusSending };
  if (low.includes("reject")) return { text: "Rejected", style: styles.statusRejected };
...
export default function LatestSubmissionsPage() {
  ...
  // ✅ client-side: involved-only + sort newest submission_date first
  const involvedRows = useMemo(() => {
    const filtered = rows.filter((r) => isInvolved(r.involved_confirm));
    filtered.sort((a, b) => normalizeDateToMs(b.submission_date) - normalizeDateToMs(a.submission_date));
    return filtered;
  }, [rows]);
  ...
  useEffect(() => {
    ...
    // fetch once, paginate client-side so counts match exactly what you show
      ...
      <main style={styles.page}>
        <div style={styles.container} className="frosted-glass-strong">
          ...
          <div style={styles.results}>
            {!loading && total === 0 && <div style={styles.noResults}>No involved submissions found.</div>}
